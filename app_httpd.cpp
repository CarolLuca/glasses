...
extern char displayMessage[128];
extern bool newMessageAvailable;

#endif // SHARED_VARIABLES_H

// Handler for receiving analysis results
static esp_err_t receive_analysis_handler(httpd_req_t *req) {
    char buf[128];
    ...
    // Copy to display message buffer
    strncpy(displayMessage, buf, sizeof(displayMessage) - 1);
    ...
    log_i("Received analysis: %s", displayMessage);
...
void startCameraServer()
{
    ...
    // Add new handler for receiving analysis results
    httpd_uri_t analysis_uri = {
        .uri = "/analysis",
        .method = HTTP_POST,
        .handler = receive_analysis_handler,
        .user_ctx = NULL
#ifdef CONFIG_HTTPD_WS_SUPPORT
        ,
        .is_websocket = true,
    ...
    if (httpd_start(&camera_httpd, &config) == ESP_OK)
    {
        ...
        // Register the new analysis handler
        httpd_register_uri_handler(camera_httpd, &analysis_uri);
    }
